...
import {
  ...
  recordOpenTrade, getOpenTrade, getOpenTradesByTicker, getAllOpenTrades, closeTrade,
} from './database.js';
...
async function mirrorTrade(action, ticker, username) {
  ...
  try {
    if (action === 'sell') {
      ...
      if (!openTrade) {
        console.log(`[INVO] No open trade record for ${normalizedTicker} by ${username} — skipping close`);
        return;
      }

      console.log(`[INVO] Closing ${normalizedTicker} (opened by ${username} @ $${openTrade.entryPrice} on ${openTrade.side.toUpperCase()} account)`);
    }

    const side = action === 'short' ? 'short' : (action === 'buy' ? 'long' : null);
    ...
    const res = await fetch(`http://localhost:${port}/api/mirror-trade`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      // For sells, pass the side and openedBy from the open trade record
      // so server.js knows which account to close on
      const openTradeForSell = action === 'sell' ? await getOpenTrade(normalizedTicker, username) : null;
      const tradeSide = openTradeForSell?.side || null;
      body: JSON.stringify({ action, ticker: normalizedTicker, source: 'invo', ...(tradeSide && { side: tradeSide }), ...(username && { openedBy: username }) }),
    });
    const data = await res.json();

    if (res.ok) {
      console.log(`[INVO] ✅ ${action.toUpperCase()} ${normalizedTicker} sent to bot`);
      ...
      if (action === 'buy' || action === 'short') {
        ...
        if (!entryPrice) {
          console.warn(`[INVO] Could not fetch entry price for ${normalizedTicker} — recording without price`);
        }
      ...
      if (action === 'sell') {
        ...
        if (!exitPrice) {
          console.warn(`[INVO] Could not fetch exit price for ${normalizedTicker} — closing without PnL`);
        }
        const closed = await closeTrade(normalizedTicker, exitPrice, openTrade.openedBy);
        if (closed) {
          const pnlStr = closed.pnlPct !== null
            ? `${closed.pnlPct >= 0 ? '+' : ''}${closed.pnlPct.toFixed(2)}% (${closed.pnlDirection})`
            : 'PnL unknown';
          console.log(`[INVO] 📊 ${normalizedTicker} closed — entry $${closed.entryPrice} → exit $${exitPrice} | ${pnlStr}`);
        }
      }
    } else {
      console.log(`[INVO] ❌ ${action.toUpperCase()} ${normalizedTicker} failed: ${data.error}`);
    }
  } catch(e) {
...
export async function startInvoPoller(invoState) {
  ...
  if (existingTrades.length > 0) {
    console.log(`[INVO] Resuming with ${existingTrades.length} open trade(s) from DB:`);
    for (const t of existingTrades) {
      console.log(`[INVO]   ${t.ticker} — opened by ${t.openedBy} (${t.side}) @ $${t.entryPrice} at ${t.openedAt}`);
    }
  } else {
    ...
    try {
      ...
      if (isFirstRun) {
        ...
        for (const item of items) {
          ...
          if (type === 'user_sold_investment' || type === 'user_closed_investment') {
            ...
            if (openTrade) {
              ...
              if (openTrade.openedBy === username) {
                console.log(`[INVO] Startup: detected pending close for ${normalizedTicker} (opened by ${username}) — executing`);
        ...
        console.log(`[INVO] Startup complete: marked ${markedCount} notifications as seen, closed ${closedCount} pending position(s)`);
      ...
      for (const item of items) {
        ...
        if (type !== 'user_added_investment' && type !== 'user_sold_investment' && type !== 'user_closed_investment') {
          continue;
        }
