...
const fetcher = (url, opts) =>
  fetch(url, { headers: { "Content-Type": "application/json" }, ...opts }).then(
    (r) => r.json(),
  );
...
const fmt$ = (v, forceDecimals = false) => {
  if (v == null) return "—";
  const n = parseFloat(v);
  if (forceDecimals && Math.abs(n) < 1 && n !== 0) return `$${n.toFixed(4)}`;
  return `$${n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
};
...
function Metric({ label, value, color }) {
  return (
    <div
      style={{ background: C.surface2, borderRadius: 7, padding: "12px 14px" }}
    >
      <p
        style={{
          fontSize: 9,
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
        ...
        <Pill
          label={crypto ? "crypto" : "stock"}
          variant={crypto ? "crypto" : "stock"}
        />
      </div>
      ...
      >
        {signal.blocked
          ? `⚠ ${signal.blocked}`
          : signal.rangeMode
            ? `↔ Range $${signal.support?.toFixed(3)}–$${signal.resistance?.toFixed(3)}`
            : signal.reasons?.[0] || "—"}
      </span>
...
function TradeRow({ trade }) {
  ...
  return (
    <div
      style={{
        display: "grid",
        gridTemplateColumns: "130px 50px 80px 1fr 80px",
        gap: 10,
        padding: "9px 16px",
        borderBottom: `1px solid ${C.borderFaint}`,
        alignItems: "center",
    ...
    >
      ...
      <span style={{ color: C.textDim }}>
        {trade.shares?.toFixed(6)} @ {fmtPrice(trade.price)}
      </span>
      {trade.pnl !== undefined ? (
        <span
          style={{
...
function RecklessToggle({ config, fetcher, API, showToast, refresh }) {
  ...
  const toggle = async () => {
    ...
    const newSettings = next
      ...
      : { minConfidence: 0.6, rsiOversold: 38, rsiOverbought: 62 };
    ...
    showToast(
      next
        ? "🔥 Reckless mode on — trades on almost anything"
        : "Normal mode on",
      next ? "error" : "info",
    );
    refresh();
...
export default function App() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      if (c) {
        setConfig(c);
        setEditConfig((prev) => (Object.keys(prev).length ? prev : c));
      }
    } catch (e) {
  ...
  const handleScan = async () => {
    setScanning(true);
    const res = await fetcher(`${API}/scan`, { method: "POST" }).catch(
      () => null,
    );
  ...
  const handleSaveConfig = async () => {
    ...
    ["minConfidence", "profitTargetPct", "stopLossPct"].forEach((k) => {
    ...
    >
        ...
        <div>
          ...
          >
            {status?.lastScan
              ...
              : "awaiting first scan"}
            {status?.tickers &&
              ` · ${status.tickers.length} tickers · alpaca paper`}
          </p>
        </div>
        <div style={{ display: "flex", gap: 5 }}>
          {status?.paperMode !== undefined && (
            <Pill
              label={status.paperMode ? "paper" : "live"}
              variant={status.paperMode ? "paper" : "live"}
            />
          )}
          <Pill
            label={status?.botRunning ? "running" : "idle"}
            variant={status?.botRunning ? "on" : "off"}
          />
        </div>
      </div>
        ...
        <Card style={{ marginBottom: 12 }}>
          ...
          >
            {[
              ...
              {
                label: "Budget left",
                value: fmt$(account.budget_remaining),
                color:
                  account.budget_used_pct > 85
                    ? C.red
                    : account.budget_used_pct > 60
                      ? C.amber
                      : C.green,
              },
            ].map((m, i) => (
          ...
          <div style={{ padding: "10px 16px 12px" }}>
            ...
            >
              <span>
                ...
                <span style={{ color: C.text }}>
                  {fmt$(account.deployed_capital)}
                </span>
              </span>
              <span>
                Limit:{" "}
                <span style={{ color: C.text }}>
                  {fmt$(account.total_budget_usd)}
                </span>
              </span>
            </div>
            <div
              style={{ height: 3, background: C.borderFaint, borderRadius: 99 }}
            >
              <div
                style={{
                  height: "100%",
                  ...
                  background:
                    (account.budget_used_pct || 0) > 85
                      ? C.red
                      : (account.budget_used_pct || 0) > 60
                        ? C.amber
                        : C.green,
            ...
            >
              {(account.budget_used_pct || 0).toFixed(1)}% used
            </p>
          </div>
        </Card>
      )}
      ...
      >
        ...
        <Metric
          label="Buy signals"
          value={buySignals}
          color={buySignals > 0 ? C.green : C.text}
        />
      ...
      {tab === "overview" && (
        <>
          ...
          >
            <Btn
              color={status?.botRunning ? "red" : "green"}
              onClick={() =>
                fetcher(`${API}/bot/${status?.botRunning ? "stop" : "start"}`, {
                  method: "POST",
                }).then(refresh)
              }
            >
            ...
            <Btn
              onClick={() =>
                fetcher(`${API}/sync`, { method: "POST" }).then(() => {
                  showToast("Positions synced", "success");
                  refresh();
                })
              }
            >
          ...
          <Card style={{ marginBottom: 12 }}>
            ...
            {Object.keys(signals).length === 0 ? (
              ...
              >
                No signals — hit{" "}
                <span style={{ color: C.green }}>Scan now</span> to start
              </p>
            ) : (
              <>
                <div
                ...
                {Object.entries(signals).map(([ticker, sig]) => (
                  <SignalRow
                    ...
                    signal={sig}
                    onBuy={async () => {
                      const res = await fetcher(`${API}/buy/${ticker}`, {
                        method: "POST",
                      });
                      if (res.success)
                        showToast(
                          `${res.paper ? "Paper " : ""}Bought ${displayName(ticker)} @ ${fmtPrice(res.price)}`,
                          "success",
                        );
      ...
      {tab === "signals" && (
        <Card>
          ...
          >
            {["Ticker", "Price", "RSI", "Top reason", "Action", "Conf", ""].map(
              (h) => (
                <span
                  key={h}
                  style={{
          ...
          {Object.keys(signals).length === 0 ? (
            <p
              style={{ padding: "24px 16px", color: C.textMuted, fontSize: 11 }}
            >
              Run a scan first
            </p>
          ) : (
            Object.entries(signals).map(([ticker, sig]) => (
              <SignalRow
                ...
                onBuy={async () => {
                  ...
                  if (res.success)
                    showToast(`Bought ${displayName(ticker)}`, "success");
                  else showToast(`Buy failed: ${res.error}`, "error");
                  refresh();
                }}
              />
        ...
        <Card>
          ...
          {Object.keys(positions).length === 0 ? (
            <p
              style={{ padding: "24px 16px", color: C.textMuted, fontSize: 11 }}
            >
              No positions — hit Sync positions if you own stocks/crypto on
              Alpaca
            </p>
          ) : (
            Object.entries(positions).map(([ticker, pos]) => (
              <PositionRow
                ...
                pos={pos}
                onSell={async () => {
                  const res = await fetcher(`${API}/sell/${ticker}`, {
                    method: "POST",
                  });
                  if (res.success)
                    showToast(
                      `${res.paper ? "Paper " : ""}Sold ${displayName(ticker)}`,
                      "success",
                    );
      ...
      {tab === "invo" && (
        <div style={{ padding: 16 }}>
              ...
              <div>
                ...
                {hlAccount.positions.map((p, i) => (
                  ...
                  >
                    ...
                    <span
                      style={{
                        color: p.side === "long" ? C.green : "#FF4444",
                        fontFamily: "monospace",
                        fontSize: 11,
                      }}
                    >
          ...
          >
            ...
            <span
              style={{
                color: invoStatus.running ? C.green : "#FF4444",
                fontFamily: "monospace",
                fontSize: 11,
              }}
            >
            ...
            <button
              onClick={async () => {
                ...
                const r = await fetch("/api/invo/test", {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ ticker, action: "short" }),
                });
                const d = await r.json();
                alert(
                  d.success
                    ? `TEST SHORT ${ticker} @ $${d.price}`
                    : `Failed: ${d.error}`,
                );
              }}
              style={{
          ...
          <div
            style={{ color: "#4A5568", fontFamily: "monospace", fontSize: 10 }}
          >
            Polls Invo every 60s • Mirrors LONG trades to LONG account • Mirrors
            SHORT trades to SHORT account
          </div>
        </div>
      )}
        ...
        <div style={{ display: "grid", gap: 10 }}>
            ...
            <div style={{ padding: 14 }}>
              ...
              >
                {[
                  ...
                  const active =
                    parseFloat(editConfig.minConfidence) === p.confidence &&
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setEditConfig((e) => ({
                          ...
                          rsiOversold: p.oversold,
                          rsiOverbought: p.overbought,
                        }));
                      }}
                      style={{
          ...
          <Card>
            ...
            <div style={{ padding: 14 }}>
              <input
                value={
                  editConfig._tickersRaw !== undefined
                    ? editConfig._tickersRaw
                    : Array.isArray(editConfig.tickers)
                      ? editConfig.tickers.join(", ")
                      : ""
                }
                onChange={(e) => {
                  isEditingConfig.current = true;
              ...
              {[
                ...
                <div
                  ...
                  style={{
                    padding: 14,
                    borderRight:
                      i === 0 ? `1px solid ${C.borderFaint}` : "none",
                  }}
                >
                  <p
                    style={{
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      isEditingConfig.current = true;
                      setEditConfig((p) => ({ ...p, [f.key]: e.target.value }));
                    }}
                    style={inp}
                  />
                </div>
              ...
              {[
                ...
                <div
                  ...
                  style={{
                    ...
                    borderRight:
                      i % 2 === 0 ? `1px solid ${C.borderFaint}` : "none",
                  }}
                >
                  <p
                    style={{
            ...
            <div style={{ padding: 14 }}>
              ...
              >
                <input
                  type="checkbox"
                  checked={
                    editConfig.paperMode !== undefined
                      ? editConfig.paperMode
                      : true
                  }
                  onChange={(e) =>
                    setEditConfig((p) => ({
          ...
          <Btn color="green" onClick={handleSaveConfig}>
            Save settings
          </Btn>
        </div>
      )}
