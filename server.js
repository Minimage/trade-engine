...
const ALPACA_SECRET =
  process.env.ALPACA_SECRET || "BRPgtEn6mbM57jirhZ4ftn4fXT8NK4QRugVL8Eaks52u";
...
async function hasPendingOrder(symbol) {
  try {
    const r = await fetch(`${ALPACA_BASE}/orders?status=open&limit=100`, {
      headers: HEADERS,
    });
...
const alpacaSym = (t) =>
  isCrypto(t) ? CRYPTO_MAP[t] || t.replace("-", "/") : t;
...
function isDoubleBottom(closes, lb = 20) {
  ...
  const mid = Math.max(
    ...s.slice(s.indexOf(min1), half + s.slice(half).indexOf(min2)),
  );
...
function isDoubleTop(closes, lb = 20) {
  ...
  const mid = Math.min(
    ...s.slice(s.indexOf(max1), half + s.slice(half).indexOf(max2)),
  );
...
function detectRange(bars, lookback = 20) {
  ...
  const support =
    sortedLows.slice(0, quarter).reduce((a, b) => a + b, 0) / quarter;
  const resistance =
    sortedHighs.slice(0, quarter).reduce((a, b) => a + b, 0) / quarter;
  ...
  if (aboveResistance)
    console.log(
      `[RANGE] ${" ".padEnd(10)} broke ABOVE resistance $${resistance?.toFixed(4)} - range invalidated`,
    );
  if (belowSupport)
    console.log(
      `[RANGE] ${" ".padEnd(10)} broke BELOW support $${support?.toFixed(4)} - range invalidated`,
    );
...
function detectSignal(bars) {
  ...
  if (ema20 && ema50) {
    total += 1;
    votes += 1; // always adds to confidence when trend is clear
    reasons.push(
      `EMA20 ${isUptrend ? "above" : "below"} EMA50 (${trendLabel.toLowerCase()})`,
    );
  }
  ...
  if (isUptrend) {
    if (isBullishEngulfing(bars)) {
      total += 2;
      votes += 2;
      reasons.push("Bullish engulfing - uptrend continuation");
    }
    if (isHammer(bars[bars.length - 1])) {
      total += 1;
      votes += 1;
      reasons.push("Hammer - uptrend reversal from dip");
    }
    if (isDoubleBottom(closes)) {
      total += 3;
      votes += 3;
      reasons.push("Double bottom - uptrend confirmed");
    }
    if (isBullishBreakout(closes)) {
      total += 2;
      votes += 2;
      reasons.push("Bullish breakout above resistance");
    }
    if (isHigherHighs(closes)) {
      total += 1;
      votes += 1;
      reasons.push("Higher highs - uptrend strengthening");
    }
    if (isDoji(bars[bars.length - 1])) {
      total += 1;
      votes += 1;
      reasons.push("Doji - pause before uptrend continues");
    }
  } else {
    if (isBearishEngulfing(bars)) {
      total += 2;
      votes += 2;
      reasons.push("Bearish engulfing - downtrend continuation");
    }
    if (isShootingStar(bars[bars.length - 1])) {
      total += 1;
      votes += 1;
      reasons.push("Shooting star - downtrend reversal from peak");
    }
    if (isDoubleTop(closes)) {
      total += 3;
      votes += 3;
      reasons.push("Double top - downtrend confirmed");
    }
    if (isBearishBreakdown(closes)) {
      total += 2;
      votes += 2;
      reasons.push("Bearish breakdown below support");
    }
    if (isLowerLows(closes)) {
      total += 1;
      votes += 1;
      reasons.push("Lower lows - downtrend strengthening");
    }
    if (isDoji(bars[bars.length - 1])) {
      total += 1;
      votes += 1;
      reasons.push("Doji - pause before downtrend continues");
    }
  }

  if (total === 0)
    return { action: "hold", confidence: 0, reasons: ["No data"], rsi, price };
  ...
  console.log(
    `[SIGNAL] ${trendLabel} ${action.padEnd(4)} conf=${(confidence * 100).toFixed(0)}% votes=${votes}/${total} | ${reasons.slice(0, 2).join(", ")}`,
  );
  return {
    action,
...
function matchTicker(alpacaSymbol) {
  ...
  const fromMap = Object.keys(CRYPTO_MAP).find(
    (k) => CRYPTO_MAP[k] === alpacaSymbol,
  );
  ...
  if (/^[A-Z]+USD$/.test(alpacaSymbol))
    return alpacaSymbol.replace(/([A-Z]+)(USD)$/, "$1-$2");
...
async function runScan() {
  ...
  for (const ticker of config.tickers) {
    try {
      ...
      if (inPos && price && price > 0 && state.startupScans > 2) {
        ...
        const posIsRanging =
          range?.isRanging && pos.avg_cost <= range.support * 1.03;

        // Determine profit target - use range resistance if in range mode, else fixed %
        const profitTarget =
          posIsRanging && range?.resistance
        ...
        const soldRecently =
          recentSell && Date.now() - recentSell < 2 * 60 * 1000;
        ...
        if ((pct >= profitTarget || safetyOverride) && !soldRecently) {
          const reason = safetyOverride
            ...
            : posIsRanging
              ? `range resistance hit ($${range.resistance?.toFixed(4)})`
          ...
          if (!soldRecently) {
            console.log(
              `[BOT] Selling ${ticker} - stop loss hit (${pct.toFixed(1)}%) - adding cooldown`,
            );
      ...
      const rangeInfo = detectRange(
        hourlyBars.length >= 20 ? hourlyBars : bars,
      );
      state.ranges[ticker] = rangeInfo;
      if (rangeInfo.isRanging) {
        console.log(
          `[RANGE] ${ticker} ranging $${rangeInfo.support?.toFixed(4)}-$${rangeInfo.resistance?.toFixed(4)} (${rangeInfo.rangeSize ? (rangeInfo.rangeSize * 100).toFixed(1) : "?"}% range, ${rangeInfo.supportTouches} support / ${rangeInfo.resistanceTouches} resistance touches)`,
        );
      }
      ...
      if (onCooldown) {
        if (isTrendReversing(bars)) {
          console.log(
            `[BOT] ${ticker} trend reversing - lifting cooldown early`,
          );
          delete state.cooldowns[ticker];
        } else {
          const remaining = Math.round((cooldown.until - Date.now()) / 60000);
          state.signals[ticker].blocked =
            `Cooldown: ${cooldown.reason} (${remaining}m left)`;
        }
      }

      // Entry logic
      const stillOnCooldown =
        state.cooldowns[ticker] && Date.now() < state.cooldowns[ticker].until;
      ...
      if (!inPos && !stillOnCooldown) {
        ...
        if (deployed + config.maxPositionUsd > config.totalBudgetUsd) {
          ...
          const sym = isCrypto(ticker)
            ? (CRYPTO_MAP[ticker] || ticker.replace("-", "/")).replace("/", "")
            : ticker;
          if (await hasPendingOrder(sym)) {
            ...
            if (!isCrypto(ticker) && !(await isMarketOpen())) {
              ...
              if (nearSupport) {
                console.log(
                  `[BOT] ${ticker} RANGE BUY near support $${rangeInfo.support?.toFixed(4)} resistance $${rangeInfo.resistance?.toFixed(4)}`,
                );
                ...
                state.signals[ticker].blocked =
                  `Ranging - waiting for support ($${rangeInfo.support?.toFixed(4)})`;
              }
            }
  ...
  state.startupScans++;
  console.log(
    `[SCAN] Done - ${Object.keys(state.signals).length} signals, ${Object.keys(state.positions).length} positions (scan #${state.startupScans})`,
  );
}
...
app.get("/api/account", (req, res) => {
  ...
  res.json({
    ...
    budget_used_pct:
      config.totalBudgetUsd > 0
        ? Math.round((deployed / config.totalBudgetUsd) * 10000) / 100
        : 0,
  });
});
...
app.post("/api/mirror-trade", async (req, res) => {
  ...
  try {
    ...
    if (!hlAsset.found) {
      return res
        .status(404)
        .json({ error: `${ticker} not found on Hyperliquid` });
    }
...
app.post("/api/invo/test", async (req, res) => {
  ...
  console.log(
    `[INVO TEST] Simulating ${action.toUpperCase()} ${ticker} on Invo account`,
  );
  try {
...
app.post("/api/scan", async (req, res) => {
  ...
  res.json({
    ...
    count: Object.keys(state.signals).length,
  });
});
